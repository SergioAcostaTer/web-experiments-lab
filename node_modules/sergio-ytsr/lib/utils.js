...
exports.parseFilters = (json) => {
  ...
  const filterWrapper =
    ((wrapper.subMenu || wrapper.submenu)?.searchSubMenuRenderer || {})
      .groups || [];
  ...
  for (const filterGroup of filterWrapper) {
    ...
    for (const filter of filterGroup.searchFilterGroupRenderer.filters) {
      ...
      if (!isSet)
        targetURL =
          filter.searchFilterRenderer.navigationEndpoint.commandMetadata
    ...
    parsedGroups.set(
      parseText(
        filterGroup.searchFilterGroupRenderer.title,
        "Unknown Category"
      ),
      singleFilterGroup
    );
...
const parseText = (exports.parseText = (txt, def = null) => {
  ...
  if (Object.prototype.hasOwnProperty.call(txt, "simpleText"))
...
exports.parseIntegerFromText = (txt) =>
  Number(parseText(txt).replace(/\D+/g, ""));
...
exports.prepImg = (img) => {
  // Resolve url
  img.forEach(
    (x) => (x.url = x.url ? new URL(x.url, BASE_URL).toString() : null)
  );
  // Sort
...
exports.parseWrapper = (primaryContents) => {
  ...
  if (primaryContents.sectionListRenderer) {
    rawItems = primaryContents.sectionListRenderer.contents.find(
...
exports.parsePage2Wrapper = (continuationItems) => {
  ...
  for (const ci of continuationItems) {
    ...
    if (Object.prototype.hasOwnProperty.call(ci, "itemSectionRenderer")) {
      ...
      Object.prototype.hasOwnProperty.call(ci, "richSectionRenderer")
    ) {
      rawItems.push(ci.richSectionRenderer.content);
      // Continuation
    } else if (
      Object.prototype.hasOwnProperty.call(ci, "continuationItemRenderer")
    ) {
      continuation = ci;
    }
  }
